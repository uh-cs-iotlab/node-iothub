...
module.exports = class extends VirtualFeedHelper(Base) {

    getAdminRoleId() {
        return this.app.models.HubRole.findOne({where: {name: 'admin'}})
        .then(adminRole => {
    ...
    createUser(userCreds, roleWhere) {
        return this.app.models.HubUser.create(userCreds)
        .then((user) => {
            if (roleWhere) {
                return new Promise((resolve, reject) => {
                    this.app.models.HubRole.findOne({where: roleWhere}, (err, role) => {
                        if (err) reject(err);
                        resolve(role);
                    });
                })
    ...
    login(userCreds) {
        return this.app.models.HubUser.login(userCreds);
    }

    removeUser(userId, tokenId) {
        var userP = Promise.resolve();
        if (tokenId) {
            userP = this.app.models.HubUser.logout(tokenId);
        }
        return userP
        .then(() => this.app.models.HubUser.destroyById(userId));
    }
