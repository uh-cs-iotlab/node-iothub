...
function checkPlugin(name, script) {
    try {
        var sandbox = {}; // Bare VM for plugins
        vm.createContext(sandbox);
        var res = vm.runInContext(script, sandbox); // bare vm
        if (sandbox.hasOwnProperty(name)) {
            var plugin = sandbox[name];
            var properties = ["needConfiguration", "checkConfiguration", "configure", "isFeatureSupported",
            ...
            for (var i = 0; i < properties.length; i++) {
                if (!plugin.hasOwnProperty(properties[i])) {
                    return false;
                }
            }
            return true;
